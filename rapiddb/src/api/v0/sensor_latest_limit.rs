...
pub fn get(
    ...
    warp::path!("api" / "v0" / String / "latest" / usize)
        .and(warp::get())
        .map(move |id: String, limit: usize| {
...
async fn test_get() {
    let database_test_factory =
        crate::db::DatabaseTestFactory::new(".temp/test/sensor_latest_limit/test_get");
    ...
    for db in database_test_factory.get_instance().values() {
        ...
        for _ in 0..limit - 2 {
            db.write().unwrap().post(
                &id,
                serde_json::json!({ "id2": &id }).to_string().as_bytes(),
            );
        }
        ...
        db.write().unwrap().post(
            &id,
            serde_json::json!({ "id3": &id }).to_string().as_bytes(),
        );
        ...
        for _ in 0..8 {
            db.write().unwrap().post(
                &id,
                serde_json::json!({ "id4": &id }).to_string().as_bytes(),
            );
        }
