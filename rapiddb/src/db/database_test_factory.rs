...
static REDIS_COUNTER: std::sync::atomic::AtomicUsize = std::sync::atomic::AtomicUsize::new(1);
...
pub struct DatabaseTestFactory {
    db_path: String,
    databases: std::collections::HashMap<String, std::sync::Arc<std::sync::RwLock<dyn IDatabase>>>,
}
...
impl DatabaseTestFactory {
    ...
    pub fn get_instance(
        &self,
    ) -> &std::collections::HashMap<String, std::sync::Arc<std::sync::RwLock<dyn IDatabase>>> {
        &self.databases
    }
}
