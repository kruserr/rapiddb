...
pub struct RedisMysqlDatabase {
    ...
    aggregates: std::collections::HashMap<String, std::sync::Arc<std::sync::Mutex<Vec<u8>>>>,
    aggregates_fn: HashMap<String, Arc<Mutex<dyn Fn(&str, &[u8], &Arc<Mutex<Vec<u8>>>) + Send>>>,
}
impl Drop for RedisMysqlDatabase {
    fn drop(&mut self) {
        if self.db_path != "0" {
            let _: () = redis::cmd("FLUSHDB").query(&mut self.redis_con).unwrap();
            self.mysql_con
                .write()
                .unwrap()
...
impl RedisMysqlDatabase {
    ...
    pub fn new_with_all(
        ...
        mysql_con
            .query_drop(format!(r"CREATE DATABASE IF NOT EXISTS rapiddb{db_path}"))
            .err();
        ...
        let mut sensors: std::collections::HashMap<String, usize> = Default::default();
...
impl IDatabase for RedisMysqlDatabase {
    ...
    fn get(&mut self, id: &str, rec_id: usize) -> Vec<u8> {
        ...
        if *self.sensors.get(id).unwrap() == 0 || rec_id > *self.sensors.get(id).unwrap() - 1 {
            return Default::default();
        }
    ...
    fn post_meta(&mut self, id: &str, value: Vec<u8>) {
        ...
        let _: () = self.redis_con.set(format!("{id}:meta"), value).unwrap();
    }
    ...
    fn get_all_meta(&mut self) -> std::collections::HashMap<&str, Vec<u8>> {
        let mut result: std::collections::HashMap<&str, Vec<u8>> = Default::default();

        for id in self.sensors.keys() {
            result.insert(id, self.redis_con.get(format!("{id}:meta")).unwrap());
        }
    ...
    fn get_all_latest_with_limit(
        ...
        let mut result: std::collections::HashMap<&str, Vec<Vec<u8>>> = Default::default();
